...
Open http://127.0.0.1:3000. Set `OPENAI_MODEL` to change the default `gpt-5-mini`. Remove the key from your shell when finished (`Remove-Item Env:OPENAI_API_KEY`). Never enter a key in browser code, Copilot Chat, screenshots, or Git. `python -m http.server` only serves the old static UI; it cannot generate AI responses.
